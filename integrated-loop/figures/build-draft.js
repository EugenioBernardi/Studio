...
**Which part of "Alzheimer's" carries the deficit** (leave-one-out from the full profile,
**Figure 4**): ${looRows.map(r => `${r.k} +${n(r.late)}`).join(", ")} recovered at one week. Discharge
capture — the mechanism this model was built on — is a minor term in Alzheimer's
(+${n(looRows.find(r => r.k === "discharge capture").late)}). It belongs to epilepsy. We report this
because it constrains the scope of the claim rather than extending it.
